...
class AsyncAopCertClient
{
    ...
    // 添加构造函数
    function __construct($http_async)
    {
        $this->http_async = $http_async;
    ...
    public function async_execute($request, $success_cb = null, $error_cb = null, $authToken = null, $appInfoAuthtoken = null, $targetAppId = null)
    {
        return $this->execute($request, $authToken, $appInfoAuthtoken, $targetAppId, $success_cb, $error_cb);
    }

    public function execute($request, $authToken = null, $appInfoAuthtoken = null, $targetAppId = null, $success_cb = null, $error_cb = null)
    {
        $this->setupCharsets($request);
        ...
        try {
            ...
            $ali_success_cb = function($resp) use($that, $request, $requestUrl, $sysParams, $success_cb) {
                //解析AOP返回结果
                ...
                if ("json" == strtolower($that->format)) {
                    ...
                    if (null !== $respObject) {
                        $respWellFormed = true;
                        $signData = $that->parserJSONSignData($request, $resp, $respObject);
                    }
                } else if ("xml" == $that->format) {
                    $disableLibxmlEntityLoader = libxml_disable_entity_loader(true);
                    $respObject = @ simplexml_load_string($resp);
                    if (false !== $respObject) {
                        $respWellFormed = true;
                        $signData = $that->parserXMLSignData($request, $resp);
                    }
                ...
                if (false === $respWellFormed) {
                    $that->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_RESPONSE_NOT_WELL_FORMED", $resp);
                    return false;
                }

                // 验签
                $that->checkResponseSign($request, $signData, $resp, $respObject);

                // 解密
                if (method_exists($request, "getNeedEncrypt") && $request->getNeedEncrypt()) {
                    if ("json" == strtolower($that->format)) {
                        $resp = $that->encryptJSONSignSource($request, $resp);
                        // 将返回结果转换本地文件编码
                        $r = iconv($that->postCharset, $that->fileCharset . "//IGNORE", $resp);
                        $respObject = json_decode($r);
                    } else {
                        $resp = $that->encryptXMLSignSource($request, $resp);
                        $r = iconv($that->postCharset, $that->fileCharset . "//IGNORE", $resp);
                        $disableLibxmlEntityLoader = libxml_disable_entity_loader(true);
                ...
                return $success_cb ? $success_cb($respObject) : $respObject;
            };

            if(!$this->http_async){
                $resp = $this->curl($requestUrl, $apiParams);
                return $ali_success_cb($resp);
            } else {
